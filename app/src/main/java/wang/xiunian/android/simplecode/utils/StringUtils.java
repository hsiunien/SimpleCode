...
package wang.xiunian.android.simplecode.utils;
...
public class StringUtils {
    ...
    public static boolean checkPwd(String pwd) {
        return !isEmpty(pwd) && pwd.length() >= 6 && pwd.length() <= 20;
    }
     ...
     * 手机号去敏 前三 后四
     */
    public static String maskMobileNo(String s) {
        if (isEmpty(s) || s.length() < 7) return s;
    ...
    public static double toYuanByFenInt(String money) {
        if (TextUtils.isEmpty(money)) {
            return 0;
        }

        money = money.replaceAll(",", "");
        if (checkMoneyValid(money)) {
            double result = new BigDecimal(money)
                    .divide(new BigDecimal("100"))
                    .setScale(2, BigDecimal.ROUND_FLOOR).doubleValue();
    ...
    public static double toFenByYuanInt(String moneyStr) {
    ...
    public static String reverse(String pk2) {
        return new StringBuffer(pk2).reverse().toString();
    }
